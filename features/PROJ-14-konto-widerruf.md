...
#### BUG-2: Konto-Icon-Header-Tests flaky auf Mobile Chrome (`.first()` griff manchmal den unsichtbaren Desktop-Link) — BEHOBEN (2026-09-09, PR #7)
...
- **Fix:** Locator in `tests/PROJ-14-konto-widerruf.spec.ts` (Tests "/ (Startseite)" und "/analyse") auf `page.locator('a[href="/konto"]:visible').first()` umgestellt, sodass unabhängig vom Viewport immer der tatsächlich sichtbare Link ausgewählt wird. Gelandet via [PR #7](https://github.com/becherohnehenkel/endlichsattapp/pull/7); verifiziert mit mehreren Wiederholungen auf `--project="Mobile Chrome"` sowie `--project="chromium"` (jeweils alle Konto-Icon-Tests grün).
- **Hinweis:** `/upgrade` hatte nie zwei gleichzeitige Links (kein `TopNav`, siehe `src/lib/nav-visibility.ts`) und war nie betroffen.
