...
- Ein vorbestehender, von dieser Änderung unabhängiger Flake im Test "Laden-Skelett erscheint während Daten geladen werden" wurde bei der Verifikation entdeckt (reproduziert auch auf dem Stand VOR diesem Refinement, `--repeat-each=5` bestätigt) — als separater Task ausgelagert, nicht Teil dieses Refinements. **Behoben 2026-09-04** ([PR #5](https://github.com/becherohnehenkel/endlichsattapp/pull/5)): `resolveDelay` wurde bislang erst im Route-Handler zugewiesen (getriggert durch den echten Request), das rein clientseitige Ladeskelett war aber oft schon sichtbar, bevor der gemockte Fetch die Interception erreichte — `resolveDelay()` konnte den Handler damit überholen. Fix: Zuweisung synchron vor der Routenregistrierung, dediziertes `data-testid="mahlzeit-historie-skeleton"` am Skelett (`src/components/mahlzeit-historie.tsx`) statt CSS-Klassen-Selektor. Verifiziert mit `--repeat-each=10` (zweimal, 30/30 grün) sowie dem vollständigen Testfile (19/19 grün).
