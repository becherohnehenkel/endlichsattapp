...
const TEXT_ALLGEMEIN = 'KI-gestützte Einschätzung — kann Fehler enthalten'
const TEXT_ECHTHEIT = 'Rezept ist echt — nur die Analyse und Zuordnung zu deiner Mahlzeit sind KI-gestützt'
...
test.describe('Rezept-Detailseite: KI-Hinweis', () => {
  test('zeigt beide Hinweise bei einem Rezept mit Sättigungs-Matrix', async ({ page }) => {
    ...
    await expect(page.getByText(TEXT_ALLGEMEIN)).toBeVisible()
  ...
  test('zeigt keinen KI-Hinweis bei einem Beilagen-Rezept (RezeptKontextHinweis statt Matrix)', async ({ page }) => {
    ...
    await expect(page.getByText(TEXT_ALLGEMEIN)).toHaveCount(0)
  ...
  test('Icon ist für Screenreader als dekorativ markiert', async ({ page }) => {
    await loginAs(page)
    await page.goto(`/rezept/${MATRIX_RECIPE_ID}`)
    const hinweis = page.getByText(TEXT_ALLGEMEIN).locator('..')
  ...
  test('Hinweise bleiben auf 375px vollständig lesbar (kein Abschneiden)', async ({ page }) => {
    ...
    await loginAs(page)
    await page.goto(`/rezept/${MATRIX_RECIPE_ID}`)
    const allgemein = page.getByText(TEXT_ALLGEMEIN)
    ...
    await expect(allgemein).toHaveText(TEXT_ALLGEMEIN)
